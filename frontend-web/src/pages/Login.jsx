...
const Login = () => {
    ...
    const handleSignup = async (e) => {
        ...
        try {
            ...
            // We'll use a signup endpoint or reuse the login one if it creates the doc
            // For now, let's assume we need a signup call
            const signupData = {
                email,
                fullName,
                username,
                role: 'technician', // Default role for self-signup
                roles: ['technician'],
                idToken
            };
            
            // Calling a hypothetical signup endpoint or reusing creation logic
            // In our current backend, we might need a dedicated public signup route
            // For this demo, let's try to login which might fail if doc doesn't exist
            // So we should ideally have a signup route.
                ...
                <div className="relative z-20 w-full h-full flex flex-col justify-between p-16 xl:p-24">
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-blue-300">Quality Control</p>
                        </div>
                    </div>
                    ...
                    <div className="max-w-md">
                        <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full bg-white/10 border border-white/10 backdrop-blur-md mb-10">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: 'white' }}>Système Certifié IA</span>
                        </div>
                    ...
                    <div className="flex items-center gap-4 text-xs font-black tracking-[0.4em] uppercase" style={{ color: 'rgba(255, 255, 255, 0.6)' }}>
                    ...
                    <div className="bg-white rounded-[60px] p-12 lg:p-20 shadow-[0_40px_100px_-20px_rgba(15,23,42,0.06)] border border-blue-50 relative overflow-hidden">
                        ...
                        <div className="mb-12">
                            ...
                            <p className="text-xl text-slate-500 font-medium">
                                {isLogin ? 'Authentification sécurisée au portail ICEM.' : 'Créez votre compte collaborateur.'}
                            </p>
                        </div>
                        ...
                        <form onSubmit={isLogin ? handleLogin : handleSignup} className="space-y-8">
                            {!isLogin && (
                                <>
                                    <div className="space-y-3">
                                        <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] ml-2">Nom Complet</label>
                                    ...
                                    <div className="space-y-3">
                                        <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] ml-2">Nom d'utilisateur</label>
                                        <div className="relative group/input">
                                            <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within/input:text-blue-600 transition-colors">
                                                <Activity size={24} />
                                            </div>
                            ...
                            <div className="space-y-3">
                                <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] ml-2">Email Professionnel</label>
                                <div className="relative group/input">
                                    <div className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within/input:text-blue-600 transition-colors">
                                        <Mail size={24} />
                                    </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex justify-between items-center px-2">
                                    <label className="block text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">Mot de passe</label>
                                    {isLogin && (
                            ...
                            <button type="submit" disabled={loading}
                                ...
                                {loading ? (
                                    ...
                                    <>
                                        {isLogin ? "S'authentifier" : "Créer mon compte"} 
                                        {isLogin ? <Zap size={20} className="fill-white" /> : <UserPlus size={20} />}
                                    </>
                                )}
                            </button>
                        ...
                        <div className="mt-12 flex flex-col items-center gap-6">
                            ...
                            <div className="flex items-center gap-3 text-slate-300 text-[10px] font-black uppercase tracking-[0.5em]">
                                <Globe size={14} />
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
                    <div className="w-full max-w-xl bg-white rounded-[50px] p-16 shadow-2xl animate-[modal-in_0.3s_ease-out] relative">
                        <button onClick={() => setShowForgotPassword(false)}
                            className="relative z-10 flex items-center gap-2 text-slate-400 hover:text-blue-600 text-xs font-black uppercase tracking-widest mb-10 transition-colors">
